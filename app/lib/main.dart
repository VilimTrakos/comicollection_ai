...
Color accentColor(String key) => switch (key) {
  'yellow' => const Color(0xFFC89125),
  'blue' => const Color(0xFF2F69B3),
  _ => red,
};
...
class _RangeEntryPageState extends State<RangeEntryPage> {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SectionTitle('STANJE ZA SVE'),
          Wrap(
            spacing: 7,
            children: ['M', 'VF', 'F', 'G', 'P']
                .map(
          ...
          FilledButton.icon(
            ...
            label: Padding(
              padding: const EdgeInsets.all(14),
              child: Text(
                saving ? 'SPREMAM…' : 'DODAJ ${_issueCount(_selectedCount)}',
                style: const TextStyle(fontWeight: FontWeight.w900),
              ),
            ),
          ),
  ...
  Future<void> _save() async {
    ...
    for (final number in _numbers) {
      final current = existing[number];
      final owned = !excluded.contains(number);
      final base =
          current ??
          Comic(
      ...
      changes.add(
        base.copyWith(
          ...
          condition: owned ? grade : base.condition,
        ),
      );
    }
    try {
